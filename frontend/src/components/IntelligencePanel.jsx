...
export default function IntelligencePanel({ isOpen, onClose, roomId }) {
        ...
        const fetchIntelligence = async () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    console.error("Intelligence fetch failed with status:", res.status);
                    setIsLoading(false);
                }
            } catch (err) {
    ...
    const generateCalendarUrl = (title, isoDate) => {
        if(!isoDate) return '#';
        ...
        const hour = parseInt(isoDate.substring(9, 11));
        const minSecZ = isoDate.substring(11);
        
        const endHour = String((hour + 1) % 24).padStart(2, '0');
        const endDate = `${year}${month}${day}T${endHour}${minSecZ}`;
    ...
    return (
        <>
            <div 
                className="md:hidden fixed inset-0 z-[90] bg-black/60 backdrop-blur-sm animate-in fade-in duration-200"
                onClick={onClose}
            />
            <div className="fixed inset-y-0 right-0 z-[100] md:relative md:z-40 w-[85%] max-w-[350px] md:w-[320px] lg:w-[350px] flex-shrink-0 bg-[#161618] border-l border-[#2c2c2f] flex flex-col h-full shadow-2xl md:shadow-none animate-in slide-in-from-right duration-200">
                <div 
                    className={`absolute top-4 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-2.5 w-[90%] bg-[#ffab00]/15 border border-[#ffab00]/30 backdrop-blur-md text-[#ffab00] px-4 py-2.5 rounded-xl shadow-[0_10px_30px_rgba(255,171,0,0.15)] transition-all duration-300 ease-out
                    ${notification 
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {data.tasks.map((task, i) => (
                                            <div key={i} className="bg-white/5 border border-[#2c2c2f] rounded-xl p-3.5 flex items-start gap-3 hover:border-[#8444f6]/50 transition-colors group">
                                                ...
                                                {task.isoDate && (
                                                        <a 
                                                            href={generateCalendarUrl(task.title, task.isoDate)}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            title="Add to Google Calendar"
                                                            className="opacity-0 group-hover:opacity-100 transition-opacity bg-[#2c2c2f] hover:bg-[#8444f6] text-[#8f8f96] hover:text-white p-1.5 rounded-lg shrink-0 flex items-center justify-center"
                                                        >
