...
export default function ChatHeader({user, activeRoom, setIsDrawerOpen, setRooms, onSelectRoom, searchQuery, setSearchQuery, setSearchTrigger, onlineUsers=[], isIntelligenceOpen, setIsIntelligenceOpen}) {
    ...
    return (
        <section className="w-full flex items-center justify-between px-4 sm:px-6 py-4 border-b border-[#2c2c2f] bg-[#0a0a0a]">
            <div 
                className={`items-center gap-3 sm:gap-4 cursor-pointer min-w-0 ${isSearching ? 'hidden sm:flex' : 'flex'}`} 
                onClick={() => {
            ...
            >
                ...
                <button 
                    ...
                    className="md:hidden p-1.5 -ml-2 text-[#8f8f96] hover:text-[#e1e1e3] transition-colors shrink-0"
                >
                ...
                <div className="flex flex-col min-w-0">
                    <h2 className="text-[15px] font-semibold text-[#e1e1e3] truncate">{displayName}</h2>
                    <p className="text-[13px] font-medium text-[#8f8f96] mt-0.5 truncate">{subtitle}</p>
                </div>
            </div>

            <div className={`flex items-center gap-2 sm:gap-5 text-[#8f8f96] ${isSearching ? 'w-full sm:w-auto flex-1 sm:flex-none' : 'shrink-0'}`}>
                {isSearching ? (
                    <div className="flex items-center gap-2 w-full animate-in fade-in slide-in-from-right-4 duration-300">
                        <input 
                            ...
                            className="flex-1 sm:flex-none sm:w-[220px] bg-[#161618] border border-[#2c2c2f] rounded-lg px-3 py-1.5 text-[#e1e1e3] text-[13px] outline-none focus:border-[#8444f6] transition-colors min-w-0"
                        />

                        <button 
                            className="shrink-0 bg-[#8444f6] hover:bg-[#7133d4] text-white text-[13px] font-medium px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 shadow-sm"
                        ...
                        <button 
                            ...
                            className="shrink-0 text-[#8f8f96] hover:text-[#e1e1e3] text-[13px] font-medium transition-colors px-1"
                        >
                            Cancel
                        </button>
                    </div>
